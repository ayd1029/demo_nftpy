<template>
        ...
        <div class="row justify justify-between q-pt-sm">
          ...
          <div class="q-pl-sm">
            <div class="row flex flex-center">
              ...
              <div v-if="getUid === tokenVo.reg_id" class="col flex flex-center">
                <q-btn flat round dense icon="edit" size="md" color="primary" icon-right="true" @click="goModify" />
              </div>
              <div v-if="getUid === tokenVo.reg_id" class="col">
                <q-btn flat round dense icon="delete" size="md" color="primary" icon-right="true" @click="deleteNftProject" />
              </div>
              <div class="col">
  ...
  <q-dialog v-model="confirmDeleteNftProject">
    <q-card>
      <q-card-section class="row items-center" style="min-width: 200px;">
      ...
      <q-card-actions align="around">
        <q-btn flat style="width: 45%;" :label="$t('cancel')" color="black" v-close-popup />
        <q-btn flat style="width: 45%;" :label="$t('delete')" color="black" v-close-popup @click="doDeleteNftProject" />
      </q-card-actions>
    </q-card>
  </q-dialog>
...
export default defineComponent({
  ...
  //     const ogUrlString = 'https://nftpy.io/info?seq=' + this.tokenVo.seq + '&tab=' + this.tab
  ...
  data () {
    return {
      ...
      confirmDeleteNftProject: false, // NFT 프로젝트 삭제 모달
    }
  },
  components: {
  },
  ...
  methods: {
    ...
    shareUrlCopy(value) {
      ...
      // API 서버로 보내서 og 정보 달고 다시 frontPath로 넘어옴. path: https://nftpy.io/#/info?seq=999&tab=XXX
    ...
    goBack() {
      try {
        ...
        this.$router.push('/nft/nftProjectList')
      }
    },
    // 나의 댓글 삭제 모달 표시
    ...
    goModify() {
      this.$router.push({ path: '/nft/modifyNftProject', query: { seq: this.tokenVo.seq }})
    },
    deleteNftProject() {
      // 삭제 확인창 표시
      this.confirmDeleteNftProject = true
    },
